...
"agent" now names only the `/v1/agent/*` platform. Verbs: `/task ls·get·watch·respond·collect·resume·cancel·help`, `/run ls·get·watch·collect·cancel·help` (no `respond`/`resume` — oneshot runs can't park). **All four client families have `/run` and `/task`** since T8b (2026-08-08): they register client-agnostically in `ppxai/commands/factory.py` (`"task"` in `_BUILTIN_COMMAND_MODULES`) — web reaches them through the fetched roster (`GET /commands`, ADR 0007 step 3a; the hand-written `ppxai/web/shared/commands.js` is deleted), VSCode still through its own `vscode-extension/src/shared/commands.ts` until step 3b. **One nuance:** `launch` and `resume` need a live asyncio loop (`_NEEDS_LOOP` in `ppxai/commands/task.py:52`), so the **Rich TUI rejects those two verbs** with an actionable error — every read verb (`ls`, `get`, `watch`, `collect`, …) works everywhere. Textual, web and VSCode have full parity.
