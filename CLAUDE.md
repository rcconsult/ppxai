...
- **One command registry — declare once, derive everywhere** (ADR 0007, all five steps landed 2026-09-21 on `bugfix/v1.19.3`, unreleased) — `CommandSpec` is the ONLY place a command is declared. **Every hand-written roster is gone** — seven live ones plus a caller-less eighth: `web/shared/commands.js`, `app.js`'s inline catalog, `vscode-extension/src/shared/commands.ts`, `_BUILTIN_SPECIAL_COMMANDS` + `_CLIENT_GATES`, the seven `_*_SUBCOMMANDS` tables, `rich/ui.py::display_welcome`'s ~30-command list (the seventh, found in step 4, **18 commands out of date**), and the dead `ui_components.py::render_welcome`. Both JS clients fetch `GET /commands?client=<id>` and route on the entry's `dispatch` field, failing CLOSED with no roster; the TUIs call `CommandFactory.roster()` in-process; Rich's welcome screen and both TUI `/quit` intercepts derive from the registry too (`CommandFactory.names_for_client_action("app.quit")`). **Adding a command means editing ONE `CommandSpec` — never a client-side list.** **Step-5 follow-ups (2026-09-21):** `/q` is now a registered `/quit` alias (Rich + Textual); `/help <cmd>` lists a command's `subcommands`; four of the five VSCode `LEGACY_INTERCEPTS` (`tools`, `context`, `ls`, `tree`) migrated to factory routing — see the guard-test table below and `docs/plan-adr-0007-completion-service.md` §"Step 5 follow-ups". → [docs/decisions/0007-completion-first-class-service.md](docs/decisions/0007-completion-first-class-service.md), [docs/plan-adr-0007-completion-service.md](docs/plan-adr-0007-completion-service.md)

**Structural guard tests — keep these green, and read the failure message; each says what to DO:**

| Test | Rule it holds |
|---|---|
| `tests/test_command_parity_fence.py` (ADR 0007 step 5) | Every `client_action` Python declares for a client is implemented by it AND vice versa (no orphans); no per-name intercept in either JS client; `LEGACY_INTERCEPTS` is a **shrinking baseline** that fails when it grows AND when it shrinks without its baseline row going too — down to **1 row** (`checkpoint`) since 2026-09-21; `/checkpoint clear` deletes every snapshot and VSCode's modal is currently the only confirmation any client has, so the owner decided (2026-09-21) on a cross-client confirmation (option B) built for all four clients — including both TUIs, neither of which consumes command `side_effects` today — before `checkpoint` migrates off this table; implementation in progress; Textual's `"q"` legacy-extra baseline is retired (`q` is now a registered `/quit` alias) and a dedicated test pins it staying gone; **no new literal command catalog anywhere** (a literal naming ≥ 6 registered commands); `SideEffectKind` coverage in both directions, derived from Python |
