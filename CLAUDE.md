...
**Active branch:** `bugfix/v1.19.1` (**not yet released**). Opened for tool-loop transcript integrity; it now also carries two implemented ADRs. See [CHANGELOG.md](CHANGELOG.md) `[1.19.1]` for the full list and [docs/release-notes-v1.19.1-DRAFT.md](docs/release-notes-v1.19.1-DRAFT.md) for the migration detail.
...
"agent" now names only the `/v1/agent/*` platform. Verbs: `/task ls·get·watch·respond·collect·resume·cancel·help`, `/run ls·get·watch·collect·cancel·help` (no `respond`/`resume` — oneshot runs can't park). **Surface split:** `/run` and `/task` exist in **web + VSCode only** — they are registered in `ppxai/web/shared/commands.js`, *not* in `ppxai/commands/factory.py`, so the Rich/Textual TUIs have `/auto` but no `/run`//`task` until the parked T8b transport decision lands.

**`execution.*` is the third top-level config axis** (ADR 0010/0011), read via `ppxai/config/execution.py`: `execution.run.{grounding,web_search}` (oneshot enrichment, both default off), `execution.profiles` (named reusable task grants), `execution.egress_ceiling` (deployment-wide egress cap), `execution.collect` (`auto|yes|no`, default `yes`). Legacy keys they supersede are dual-read. `execution.oneshot.*` was renamed to `execution.run.*` before implementation — **no code reads `execution.oneshot`**.

**Agent platform (ADR 0003 Stage 2)** shipped in v1.19.0: `/v1/agent/*` run registry, tool-capable sandboxed tier, Increments 1–9 + hardening §A–§K, and the `/task` family T1–T7 (web) + T8a (VSCode). **T8b (TUI port) is ⏸️ PARKED (2026-07-07)** pending the transport decision — the TUIs are in-process with no server channel, so either the registry+runner embed in-process (recommended) or the TUIs grow an HTTP client; resume checklist in [docs/plan-task-command-sequencing.md](docs/plan-task-command-sequencing.md) §T8. T9 (container tier-d) stays deferred.
