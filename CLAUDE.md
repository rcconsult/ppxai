...
| `tests/test_consumer_import_surface.py` (ADR 0007 step 5) | Importing the out-of-repo consumer surface (v1 gateway + `engine.completion`) loads **zero** `ppxai.commands` modules — measured in a subprocess, with a planted-violation control and a crashed-probe guard so a broken probe cannot read as clean |
