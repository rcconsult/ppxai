...
Tests: **5,742 collected — 5,741 passed, 1 skipped, 0 failed** on macOS/Unix with `uv sync --all-extras` (verified 2026-09-05 on `bugfix/v1.19.1`, working tree on `cf2c1a72`, 348s). The count is environment-dependent: a base venv without the `[data]`/multipart extras skips the office + upload suites, and the release script's own run reports whatever its env yields — so the README badge routinely trails this number. On Windows the count is 5,851 passed / 32 skipped / 0 failed (verified 2026-09-14 on `bugfix/v1.19.3` @ `ef6c56b0`, 581s): the extra skips are platform gates -- `test_terminal_pty.py` (PTY is Unix-only), symlink cases in `test_filesystem_policy.py`/`test_tool_security.py` (needs Developer Mode), and POSIX signal semantics in `test_shell_tool.py`.
