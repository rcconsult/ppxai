...
> **Current Version**: v1.17.0 (March 2026) | RightPanelFrame + Web App Refactor
...
- **libghostty SDK** — `libghostty-vt` is a zero-dep library (not even libc) extracted from Ghostty's core, providing VT sequence parsing + full terminal state (cursor, styles, wrapping, scrollback, reflow). **Zig API complete**, **C API in active development** (focus after Ghostty 1.3 release). Features: SIMD-accelerated parsing (>100 MB/s), DFA parser (14 states, O(1) transitions), Kitty Graphics Protocol, Kitty Keyboard Protocol key encoder, tmux control mode, terminal-to-HTML encoding (clipboard ops). Platforms: macOS, Linux, Windows, WebAssembly (x86_64 + aarch64). Future `libghostty-<x>` libs planned: input handling, GPU rendering (OpenGL/Metal), GTK widgets, Swift frameworks. **Relevance to ppxai:** could replace terminal detection heuristics, provide proper Kitty keyboard protocol negotiation (our Textual issue #6074), and enable rich terminal-to-HTML export for session sharing. See [mitchellh.com/writing/libghostty-is-coming](https://mitchellh.com/writing/libghostty-is-coming), [ghostty.org/docs/vt/reference](https://ghostty.org/docs/vt/reference), [PR #8840](https://github.com/ghostty-org/ghostty/pull/8840)
