...
- **`GET /commands?client=<id>`** serves the full roster. Both JS clients fetch it at startup and route on the entry's `dispatch` field; the hand-written `ppxai/web/shared/commands.js` and `vscode-extension/src/shared/commands.ts` are deleted outright, along with five more hand-written command lists across the tree (a Rich welcome-screen catalog that had drifted 18 commands out of date, among them).
- **⚠️ User-visible, web/VSCode: `/quit` no longer exists as a command.** Ending a client session is a UI workflow, not a slash command — web's header button is now labelled **"Leave"** (stops the local server / closes the tab; returns to login in a hosted session), and VSCode uses its existing connect/disconnect commands. `/quit` and `/exit` still work in the Rich and Textual TUIs.
- **Both JS clients fail CLOSED on slash commands when the roster can't be fetched** — e.g. web assets from a newer build talking to an older server that doesn't yet serve `GET /commands` the same way. Commands refuse with a clear error rather than silently doing nothing or guessing at a name. Web assets are served from `~/.ppxai/web`.
- **`/token set <value>` no longer risks leaking the secret to the server.** `/token` is declared `client_handled` (state lives in the browser's `localStorage` / VSCode `SecretStorage`, not on the server), so a typed-inline `/token set <bearer>` is intercepted and masked before it ever reaches the roster-dispatch path that would otherwise POST it to `/command/token`.
- **Step-5 follow-ups, same day:** `/q` is now a registered alias of `/quit` (works in Rich too, not just Textual); `/help <cmd>` renders a "Subcommands" section for commands that declare them; `/tools help` and `/tools help editing` now work in Rich, Textual and web (previously VSCode-only); VSCode's `/tools`, `/context`, `/ls`, `/tree` and (see below) `/checkpoint` output is now server-rendered via `POST /command/<name>` (format differs slightly from the old client-side rendering; `/ls "dir with spaces"` takes only the first token, matching web's existing behaviour). `/context clear` no longer leaves a stale Ctx% badge in web/VSCode; three stale `/tools agent` hints (removed by ADR 0011) are fixed.
- **⚠️ User-visible: `/checkpoint clear` asks first, in all four clients.** `/checkpoint clear` irreversibly deletes every file-backend checkpoint; VSCode's modal used to be the only confirmation any client had, so web, Rich and Textual cleared at once. With no flag it now returns a quick-pick list — Cancel first (default-highlighted), the destructive row second — so a bare Enter can never destroy anything; Cancel re-dispatches `clear --no` (deletes nothing), the second row re-dispatches `clear --yes` (clears). Scripted use needs `--yes` directly. **Both TUIs consume `CommandResult.side_effects` for the first time** (Rich: a numbered list, Enter/`0`/non-numeric/Ctrl-C all cancel; Textual: a `QuickPickDialog` modal, Escape dismisses), which also fixes two previously dead-ended prompts: `/show @<term>` with several matches now shows the pick and opens the chosen file instead of just printing a count, and `/edit <missing-file>` now offers to create it instead of just saying "not found". VSCode's legacy intercept mechanism (`LEGACY_INTERCEPTS`, `LEGACY_HANDLERS`, the router's legacy branch, `handlers/commands.ts`, `handlers/types.ts`) is deleted outright — `/checkpoint` was its last row. VSIX size: 136 KB.

### Added — VSCode AppState types are generated; a connect-time schema check (2026-09-21, later the same day)

Closes ADR 0007's open owner decision 5. `interface AppStateFields` in `vscode-extension/src/appState.ts` was hand-written and had silently drifted from `ppxai/engine/app_state_schema.json`: the schema declared 22 fields, the interface 20 — `lastMessageRole` (v1.18.0) and `modelSupportsVision` (v1.18.6) were never added, so no VSCode code could read either by name, while the web client has gated its attach-button badge on `modelSupportsVision` since v1.18.6.

- **`vscode-extension/scripts/sync-schema.js` now also emits `src/appState.generated.ts`.** The hand-written interface is deleted; the generated file is tracked and byte-stable, pinned by `tests/test_app_state_generated_types.py` (regenerated in memory on every test run, fails on any difference). Adding a field to the schema can no longer leave the VSCode type behind.
- **New, user-visible: VSCode checks the connected server's AppState shape on every (re)connect.** `vscode-extension/src/schemaGuard.ts` fetches `GET /schema/app-state` and compares it with the extension's bundled schema. A matched pair is silent. **A newer server's extra fields are adopted for the connection silently** (logged once, never a toast) — the state is stored, not rendered, since rendering code ships compiled into the extension. **A field the extension was compiled against going missing, or changing type, on the server produces exactly one visible warning**, naming the fields and both versions. An older server with no `/schema/app-state` endpoint (pre-v1.17.4) changes nothing — one log line, chat keeps working; state must not fail closed the way the command roster does, because `AppState` is constructed before any server exists. VSIX 138 KB.

Not built at the time: a VSCode-side use of the now-typed `modelSupportsVision` (web already gates its attach badge on it) and a web-side equivalent of the connect-time check (a tab survives a server restart without a page reload, but only `GET /state` is re-fetched on that path, never the schema) — both filed as open owner decisions in [docs/plan-adr-0007-completion-service.md](docs/plan-adr-0007-completion-service.md). **Both landed later the same day — see below.**

### Added — VSCode vision badge parity, web schema-drift check, schema versioning (2026-09-21, later the same day)

Three more of ADR 0007's open owner decisions closed the same day, plus one dev-facing test-isolation fix found while closing them.

- **VSCode's attach-button vision badge now updates immediately on a command-palette provider or model switch.** The premise behind "VSCode has no vision-badge behaviour" (above) was wrong — the webview has had its own untyped mirror (`activeModelSupportsVision` in `media/webview/main.js`) since v1.18.6, so the missing generated TS type never cost it the feature. What WAS broken: `chatPanel.ts::_reanchorFromServer` updated only the extension-host `AppState` and never told the webview, so its badge stayed stale until the next chat turn; and `ppxai.switchProvider`/`ppxai.switchModel` refreshed only `/status`, which doesn't carry `model_supports_vision`. Both are fixed — the re-anchor now `postMessage`s the change to the webview, and a new `reanchorState()` is called from both palette commands, mirroring web's existing `handleProviderChange`/`handleModelChange`. The attach-time image warning's wording is also corrected: it no longer promises the image "will be sent as a text placeholder" (retracted from web back in v1.19.0/Item 24) — it now says a vision sidecar or the shell tool may handle it, or the send is blocked; never silently dropped or described from a placeholder. The gate decision itself lives in a new pure module, `vscode-extension/media/webview/visionGate.js`, unit-testable under plain Node.
- **Web now re-verifies the AppState schema when a tab reconnects.** `PpxaiApp._reanchorFromServer()` gained an opt-in schema check, run only at a reconnect boundary (heartbeat recovery, tab regaining visibility) — never on first load or a plain provider/model switch. It fetches `GET /schema/app-state` and classifies it with a new pure module, `ppxai/web/shared/app-state-schema-diff.js`, a JS re-implementation of VSCode's `schemaGuard.ts` classifier held to identical verdicts by a cross-language parity test. A matching schema is silent; a newer server's extra fields are adopted quietly; **a field this tab expected going missing or changing shape on the server shows one chat notice** ("The server changed while this tab was open … reload the page to get the matching UI."); an unreachable schema endpoint logs once and blocks nothing.
- **The AppState schema's `"version"` field is now maintained**, starting at `"1.1"` (`"1.0"` sat unchanged across five earlier field-adding commits and is treated as an unmeasured historical marker, not a real signal). MAJOR bumps for a field removed/renamed/retyped, MINOR for a field added or its default changed — enforced against a new append-only `ppxai/engine/app_state_schema_history.json`, not just documented. Neither the VSCode nor the web schema check treats `version` as the verdict; both still decide on the actual fields and report `version` only as context.
- **SchemaGuard's diagnostic log now goes to the extension's existing "ppxai HTTP" output channel** instead of only the Extension Host console, which most users never open.
- **(Developer-facing) The test suite can no longer write into the developer's real `~/.ppxai`.** Found while auditing the checkpoint-clear confirmation work: the suite had been leaving ~100 empty checkpoint directories per run (measured cumulative total on one host: ~14,900, all but two empty), appending to the `/cost` usage-events sink, interleaving into real debug logs, and — despite an existing in-process guard — a spawned `ppxai-server` smoke-test subprocess was still rewriting the real TUI session-restore pointer, because that guard only ever covered the pytest process itself. Fixed by pointing `HOME` at a throwaway directory before the first `ppxai` import, which is the one point at which redirecting `HOME` actually works. Separately, a session that never checkpoints no longer leaves an empty `sessions/checkpoints/<session>/` directory behind at all — the directory is now created lazily, on the first real snapshot.

### Changed
- **New installs no longer offer Perplexity models that die with the 2026-09-27 retirement.** `install.sh` and `scripts/install.ps1` stop writing `sonar-pro` and `sonar-reasoning-pro` model entries into a generated config. The VSCode extension's built-in Perplexity model list drops `sonar`, `sonar-pro`, `sonar-reasoning` and `sonar-reasoning-pro`, all chat-completions only, keeping `perplexity/sonar`. `.env.example` stops recommending `sonar-pro`. The `ppxai/__init__.py` usage example sets `perplexity/sonar`. Pricing rows stay, so older sessions still cost correctly. An existing config naming a retired id is unchanged; `/doctor` already flags it. History (benchmarks, docs, deprecation and facts tables) is left as written.
- **Tool calls collapse into one strip per assistant turn.** The transcript already grouped tools, but the engine emits one group per tool-loop *iteration* and an agentic run is usually one or two tools per iteration — so a twelve-iteration run stacked twelve collapsed strips between the prompt and the answer, each inserted above the assistant message and pushing the answer further down. There is now a level above that: one `tool-turn` row per turn, labelled with running counts (`14 tools · 8 steps ✓`), holding every iteration group. Collapsed by default; expanding it shows the steps, and expanding a step shows its tool bubbles. `/auto`'s per-iteration `━━━ Iteration n/m ━━━` system message folds into the same row instead of competing with it, and still appears on its own when a run uses no tools. Ported to the VSCode webview in the same change, since the two transcripts are line-for-line twins.
- **Tool bubbles are keyboard-operable and announce their state.** The disclosure headers were mouse-only, and there was no `aria-expanded` anywhere in the web app; they are now `role="button"`, tab-reachable, toggled with Enter or Space, and carry `aria-expanded` plus `aria-controls`.
- **The shipped microk8s coder template (`deploy/examples/microk8s/server-config.yaml`) is now v1.19.3-shaped.** It predated ADR 0010 and ADR 0012: no `execution` block, no `network.ssl`, and tool capability still declared per provider (`capabilities.native_tool_calling` / `tool_calling.mode`) — keys v1.19.1 silently ignores. Every model now carries a complete facts block; `execution.{run,collect,task,default_subagent}` and `network.ssl` are shown explicitly; Perplexity moves to the Responses-wire ids (`perplexity/sonar` plus the two gateway models) ahead of the 09-27 retirement, `tools.web_search.perplexity_model` included. Because the template enables the task tier with a default `web_search` grant, it now also shows the matching posture rather than the open default: `execution.task.sandbox.enforcement=in_process` with `/workspace` as the readable root and the standard deny list, and `execution.egress_ceiling=[api.perplexity.ai]` — the only host that grant needs. Both are deployment decisions an operator should make deliberately, so the template makes them visible. The `vllm-qwen36` entry is renamed `vllm-qwen38` with the 3.8 model id, matching the deployed ConfigMap. Example only — nothing in an existing install changes.

### Fixed
- **A tool bubble's chevron could expand to nothing.** The ▶ chevron rendered on every tool bubble, but the details pane behind it was only built when verbose tool output was on — so with verbose off, clicking a bubble toggled a class and revealed an empty box. Details are always built now and verbose means "start expanded", which is what the VSCode webview always did. A bubble with genuinely no payload renders no chevron at all.
...
- **The web_search tool's `perplexity_model` default now survives the 2026-09-27 Sonar retirement.** `web_search_perplexity` already resolves its wire per-model via `ModelFacts.wire_protocol` (ADR 0012 W3), but the code default was still the chat-wire id `sonar` — so anyone who never set `tools.web_search.perplexity_model` would have lost web_search on 09-27 with no config change of their own. The default is now `perplexity/sonar`, verified live on the Responses wire (2026-09-16); `ppxai-config.example.json` updated to match.
- **A stray `PydanticSerializationUnexpectedValue` warning on every Responses-wire web_search call is gone.** Perplexity's `search_results` output item isn't a type the openai SDK's `Response.output` union knows, so calling `response.model_dump()` to read it made pydantic re-validate the mismatch and print a warning to stderr on every live call. `_responses_answer_and_citations` now reads `response.output` directly (attribute access, no serialization) instead of dumping the whole typed response to a dict.
- **`/doctor`'s endpoint probe bypassed the outbound TLS resolver.** Every other outbound client obtains its verification setting from `config.tls.tls_verify()` (the v1.19.1 additive-CA resolver honouring `SSL_VERIFY` / `SSL_CERT_FILE` / `network.ssl.*`); `_probe_provider_endpoint`'s `httpx.Client` passed no `verify=` at all, so `/doctor` reported `CERTIFICATE_VERIFY_FAILED` for a corporate-CA endpoint the providers themselves reached fine (measured 2026-09-16). The probe now passes `verify=tls_verify()`.
- **`/doctor`'s facts scan (ADR 0012) could report on the wrong config file.** `_format_facts_section` and the `facts_config` helpers it calls (`migration_plan`, `misplaced_fields_in_config`, `wrong_typed_fields_in_config`, `incomplete_blocks_in_config`) took no config argument and always walked whichever config the process had loaded globally — independent of the file `audit_user_config` actually audited. Observed 2026-09-16: the facts section listed providers from a different `ppxai-config.json` than the one named in the audit header. Each helper now accepts an optional `config_data` dict, and `handle_doctor` threads through the same raw config the ADR 0010 migration scan already reads, so both sections describe the one file `/doctor` names.
- **The shipped facts row for the 27B-FP8 Qwen line now covers Qwen3.8.** The codeai and in-cluster deployments were upgraded in place to `Qwen/Qwen3.8-27B-FP8{,-agent}` (the 3.6 ids stay served as aliases). The shipped glob was `Qwen/Qwen3.[56]-27B-FP8*`, so a config naming the real 3.8 id landed on the UNMEASURED floor — prompt-based tools, no vision — while the alias id kept native tools. The glob is now `3.[568]`; the row is inherited from 3.5/3.6 (same family, parser and endpoint) pending a 3.8 benchmark run, and the vision/native test pins both 3.8 ids.
- **The context-window badge overshot 100% in tool loops** — 350% and 410% observed live (2026-09-16) where the model was really at 26% and 41%. `chat_with_tools` sums every iteration's usage into `accumulated_usage` for billing (correct — each iteration is a real, billable provider call), then called `session.update_usage(accumulated_usage, ...)` once at the end; `_sync_usage_to_state` treated that iteration-summed delta as "tokens currently in `session.messages`", over-claiming by a factor of N for an N-iteration loop. `update_usage()` gains a `context_tokens` keyword — the tool loop now passes the LAST request's own `prompt_tokens + completion_tokens` there, and `_sync_usage_to_state` uses it as the context baseline when present. Session/cost totals are unaffected; only the context badge's baseline changes. The plain (non-tool-loop) chat path is untouched — it never passes `context_tokens`, so it keeps the pre-existing single-request delta behavior.
- **Tool-loop detection now catches paraphrased repeats, not just byte-identical streaks.** `is_tool_loop_detected` used to count only a *trailing streak* of byte-identical arguments; a live incident (2026-09-22, web client) ran `web_search` 15 times in 113 seconds hunting one IMDb image id, four of the queries byte-identical but interleaved with ten paraphrases, so a streak of 3 never formed and "Loop detected" logged zero times. The guard now counts successful occurrences of `(tool, args)` anywhere in the turn — the old streak rule is deleted outright rather than kept alongside it, since a streak of N is also N occurrences and could never fire first. A second, argument-independent per-turn call budget (`tools.agent.tool_call_budgets`, default `{"web_search": 10, "fetch_url": 10}`, merged over the shipped default so raising one tool's cap doesn't uncap the others; absent or 0 = unlimited; nothing else is capped) catches the paraphrase case exact-match matching structurally cannot see — sized from tau-bench's published trajectories (1,960 real runs scanned locally: one tool called once in 83.6% of turns, five-or-fewer in 99.3%, but 32 of the 76 turns that called one tool six-plus times succeeded). Failed calls count toward neither guard, so a retry after a transient failure isn't punished — that stays the zombie breaker's job. Each guard's refusal carries its own message (a budget trip no longer says "same args", which would teach the model to rephrase — the exact behavior that caused the incident), and a second attempt at an already-exhausted tool withdraws tools for the rest of the turn and forces a synthesis pass. `GET /tools/status` and `GET /agent/config` both gain a `tool_call_budgets` key in their response body, additively; `POST /v1/oneshot` is unchanged.
- **A `/task` run's `events.jsonl` now records degraded turns.** A `turn_degraded` record is written for each tool-guard degradation (budget exhausted, tools withdrawn, repeat loop) and a `turn_end` record per turn carrying `degraded`/`degradation_reasons`. Previously the runner persisted only tool calls, so a degraded turn looked complete in the audit file. Additive; existing records and the run's own `agent_run_complete`/`agent_run_error` are unchanged. A missing `turn_end` means the turn's outcome is unknown.
- **Every tool-loop exit now ends with a run terminal.** Three `chat_with_tools` exits (prompt-based fallback provider error, retry-synthesis provider error, tool interrupt) returned without `AGENT_RUN_ERROR`; they now emit it, and the `/task` runner waits for it before failing the run (same FAILED status and message; no tool runs after an error). `agent_runs.CATEGORIES` now declares `filesystem`, which `path_denied` records already used. The web and VSCode task views render `turn_degraded` and `turn_end`, showing an unreported outcome as unknown, never clean.
- **A tool loop that hits its iteration cap now answers instead of giving up** (debt Item 80). The cap used to end the turn on a canned *"[Tool iterations limit reached. Please try again with a simpler query.]"*, throwing away everything the loop gathered. That was the only outcome for a loop neither guard can see: two tools alternating with fresh arguments every call (`read_file`, `list_directory`, `read_file`, …). After the last tool iteration, one extra pass now runs with tools withdrawn, and its answer ends the turn. The cap is a new degradation reason, `ToolGuardReason.ITERATION_CAP` (`"iteration_cap"`); the turn's `AGENT_RUN_COMPLETE` carries it in `degradation_reasons` and still sets `max_iterations_reached: true`. A `/task` audit file gets a `turn_degraded` record for it like any other guard, with `call_pattern` stored as JSON (the audit clamp now recurses into dicts instead of storing a truncated repr). At ~70% of the cap (models with a cap of 5 or more), the model gets one "converge" notice: an INFO event with `metadata.notice = "iteration_warning"`. It is a nudge, not a degradation, and refuses nothing. The cap, the notice, both guards and the zombie breaker now log the turn's **call pattern**: its shape (`cycle`, `streak`, `repeated_args`, `mixed`), the repeating tool cycle if any, per-tool call and distinct-argument counts, and the last 12 tool names. The pattern is also in the cap and notice metadata as `call_pattern`. The log lines need `/debug-log on`; the metadata does not. A detector that refuses the alternating shape is still not built: legitimate exploration looks the same. The logged pattern is the evidence a future one would be designed from.
...
| Removed | Use instead |
|---|---|
| `/agent <task>`, `/agent on\|off` | **`/auto`** — same in-session autonomous loop, checkpoint/undo intact |
| `/tools agent` | **`/tools auto`** |
| `/task run "<desc>" …` | **`/task "<desc>" …`** — direct launch; a first token counts as a verb only when followed by a run id (`run_` + 12 hex) or nothing |
| `task show` (canonical) | **`task get`** (`show`/`open` still accepted as aliases) |
| `task ack` (canonical) | **`task collect`** (`ack` still accepted as alias) |
| `/agentrun <task>` | **`/run <prompt>`** — same async one-off, now `kind=oneshot` on the full run gears; **no flags** (the grant is config-decided via `execution.run.web_search`) |
...
- **The in-process `/task` path bypassed every task-tier policy gate.** T8b gave the TUIs an in-process route to the same runner the HTTP API drives, and it reached `build_task_runner` without the route's admission checks. Concretely: `ppxaide` could start a tool-capable run while **`execution.task.enabled` was false**; a grant containing **`execute_shell_command`** evaded the server's explicit rejection (shell is the one tool no in-process control can contain — the egress allowlist cannot inspect what `curl` does, and the filesystem seal confines *named path-taking tools* by inspecting their path kwarg, which a shell command string does not have); and **`--skill` values were passed straight through as filesystem read roots**, so `--skill /etc` mounted that directory into the run's read scope under the seal. The suite was fully green throughout — no test drove one request through both paths.

  Fixed at the root rather than per call site: admission now lives in one engine-level boundary, **`ppxai/engine/task_authorizer.py::authorize_task()`**, which every client passes through. The HTTP route is a thin adapter that maps `TaskAuthorizationError` onto `HTTPException` (same statuses, same detail text). `InProcessTaskBackend.launch()` now takes an **`AuthorizedTask`** — the only way to obtain one is to pass the authorizer — and its `extra_read_paths` kwarg is **deleted**, not merely validated: read scope is an *output* of authorization (resolved skill directories confined to `skills_dir`), never a caller input, mirroring the HTTP request model which has no such field.

  Defense in depth: `build_task_runner` now rejects a shell grant **eagerly**, at the single construction site, because resume rebuilds from a *persisted* grant and `spawn_subagent` builds children through the same builder — neither re-authorizes. The tier gate deliberately does **not** move there: that builder is shared with the tool-free tier (`/v1/oneshot`, the enriched `/v1/agent/run` branch), which is documented as always available regardless of `execution.task.enabled`, so gating it would break a stable v1 surface. `InProcessTaskBackend.resume` — the one other admission path — checks the tier explicitly instead.

  Regression fence: `tests/test_task_authorization_parity.py` drives the same request through every admission path and asserts identical refusals, plus that **no run is minted** on refusal. The existing parity harnesses are static source greps and could not have caught this. Reported in `docs/archive/branch-review-v1.19.1.md` (Critical + two of the three High findings).

- **The one-off tier (`/run`) went through a second, weaker admission path.** The in-process `/run` hand-built its authorization result, so it applied none of the HTTP route's rules; the HTTP route in turn held a route-layer copy of the enrichment egress assembly. Both tiers now pass **one** `authorize()`, with the differences between them expressed as **data** (`TierPolicy` rows in `TIERS`) rather than as a second code path: `gated_by`, `grant_source`, `config_grant`, `allows_empty_grant`, `honors_client_fallback`, `validates_provider`, `iterations`. Everything below the grant — shell reject, operator kill-switch, provider validation, egress assembly, ceiling — is shared unconditionally. `AuthorizedTask` now has exactly **one construction site** in the production tree (the `return` inside `authorize()`), which is asserted by a test: a hand-built literal is how the bypass existed in the first place.

  Three defects fell out of the merge, none of them in the review:

  - **`tools.web_search.enabled=false` did not cover `/run`.** The operator kill-switch was a task-tier check, so a config-assembled `{web_search}` grant ran despite an operator's explicit veto. It now applies to every tier.
  - **The in-process `/run` used the chat pane's provider/model.** ADR 0003 §9 makes a sub-agent's provider *per-run injected intent*; a run must resolve it from the request or `execution.default_subagent`, never from the interactive UI. Offering UI context to that tier is now **refused**, not silently dropped — a quiet drop would leave the policy field decorative.
  - **`ONESHOT_SEARCH_ITERATIONS` lived in the route layer** although it is part of the grant that config decides. It is now `TIERS["oneshot"].iterations`; `server/routes/oneshot.py` re-exports it.

  The table is compiled rather than operator-described on purpose: `grant_source` and `allows_empty_grant` decide whether a request can widen its own privileges, so a JSON typo there would be a privilege escalation no test could catch. The capability *switches* stay config (`execution.task.enabled`, `execution.run.web_search`); the *invariants* are code. If tiers ever become operator-describable, the JSON must be narrowing-only against the compiled row — the intersective rule `execution.egress_ceiling` already uses.
...
- **`/task` with no grant now guides instead of firing a doomed HTTP 422 (Item 57).** A bare `/task "<desc>"` (no `--tools`/`--spec`/`--skill`/`--profile`) is correctly rejected — a tool-capable run must carry an explicit grant — but the web + VSCode clients echoed a raw *"❌ Task rejected: HTTP 422"*, read as an outage. Both clients now short-circuit an empty grant client-side with actionable guidance (add `--tools web_search`, or use `/run` for a tool-free answer); the server 422 message also names the flags a user types.
- **`/task web_search` no longer silently loses live data under a narrowed task egress (Item 59).** With a soft `preferred: perplexity` (no `strict`) and a task allowlist permitting only `api.perplexity.ai`, the shared resolver still enumerated the DuckDuckGo fallback, the AC-2 all-of rule then denied the whole call over the unreachable DDG host, and the model fabricated an answer with a disclaimer. `authorize()` now threads the run's own egress predicate into the resolver, which drops any backend the run can't reach from **both** the candidate chain (`web_premium` reads the same predicate off a per-call `ContextVar`) and the enumerated egress superset. Selection and egress can no longer diverge under a narrowed task allowlist, so `--tools web_search` works over perplexity alone with no `strict` pin required. Pure narrowing — unconfined runs (chat) are unchanged, and a run that allowlists no known backend still fail-closes.
- **rtk (and any shell wrapper) no longer goes permanently silent when its binary isn't on PATH at first check (Item 56).** `WrapperBase.is_available()` memoized `shutil.which` on the first call and never re-checked, so a startup-ordering window where the binary was briefly absent left the wrapper inactive for the whole process lifetime — commands ran raw with no rewrite and no log line (observed in a long-running coder server pod). Negative results are no longer cached; only a positive hit is memoized (a binary found once effectively never disappears mid-process, and `shutil.which` is cheap on the miss path), so a late-arriving binary is picked up on the next call with no external invalidation.
...
**`GET /agent/config` changed shape with it** (it returns that config verbatim): the six tier keys are no longer in the response. This is an internal endpoint — not part of the `/v1/*` stability contract — and its only consumer is the bundled VSCode extension, which ships versioned with the server. **Deployment note:** any deployment that pins these keys in a k8s ConfigMap or a checked-in `ppxai-config.json` must rename them in the same window — there is no grace period, and a stale key is silently ignored. `/doctor` prints the mapping. (ppxai-sre was named here in an earlier draft; verified 2026-08-15 to carry no ppxai config keys, so it is unaffected.)
...
- **Execution profiles + `enrichment`** (ADR 0009 step ③). `execution.profiles.<name>` is an AgentSpec-shaped mapping (same fields and normalizer as a `--spec` file); a run selects one with `--profile <name>` (web + VSCode `/task`) or `"profile"` on `POST /v1/agent/task`. Precedence `request > spec > skills > profile > default_subagent`; list fields (`tools`, `network`) **replace, never union**, so a more specific layer can genuinely narrow a grant (skills still union theirs in — mounting capability is their purpose). **`enrichment: true|false`** is first-class and tri-state (absent = inherit): effective-true derives `web_search` plus the full backend-superset egress baseline once, after resolution; a more specific explicit `tools` list omitting `web_search` under effective enrichment is a pre-start **400 naming both layers**, never a silent closed-book "enriched" run. `--enrichment on|off` is also a per-run `/task` flag.
- **`execution.egress_ceiling`** — deployment-wide egress cap, config-only, intersective, unset = no cap. Applied where every run's allowlist is assembled (`/task`, `/run`, the `/v1/oneshot` facade). An enriched run whose ceiling strips **every** search backend fails pre-start (400 naming the stripped hosts); a malformed ceiling is a loud 400, never a silent no-cap.
- **Enriched `/v1/oneshot`** (ADR 0009 step ①, default off). Two independent switches under `execution.run.*`, both default **off** — off/off is byte-identical to v1.19.0 and air-gap-safe with a local provider. `execution.run.grounding` selects provider-native search (Gemini grounding, Perplexity Sonar) and supersedes `tools.web_search.oneshot_grounding` (still honored via dual-read). `execution.run.web_search` gives the model exactly one tool, `web_search`, and executes the request as an auditable `kind=oneshot` registry run; the response gains an additive `grounding` field (`searched`, `queries`, `backend`, `search_cost`, `run_id`). Native wins when both are on (never double retrieval). Exists so **local models get context enrichment**. Per-combination behavior table in [api-gateway.md](docs/api-gateway.md); `/doctor` reports the effective path per configured model. Revises ADR 0004's "no tool loop in oneshot" (opt-in, perimeter preserved).
- **`execution.collect`** — one global key for the `/run` + `/task` families (default **`yes`**, the shipped T6 behavior). **`auto`**: a finished run merges its result into the active session automatically (runs auto-finalize; the watching client merges once, on completion — reopening an old run never re-merges). **`yes`**: the run holds its result (📬) until you collect it, and *collect now = finalize + merge*, so the model sees it on your next turn. **`no`**: collect is impossible — the Collect button renders disabled with an enable hint, the `collect` verb warns, and no merge path exists. Merge is **plain**: the run enters the conversation as an ordinary user(task) → assistant(result) exchange, no provenance tagging. (The pair shape is deliberate — session alternation-fixing silently drops a lone leading assistant message and collapses same-role neighbors, so a single-message merge could vanish from the next provider request; caught in the live trial.) New wiring: `GET /config/execution` and `POST /sessions/merge-run-result` (owner-guarded for remote callers; loopback keeps the UI exemption's on-the-host trust basis).
- **Per-tool egress baselines** (ADR 0009 step ②). `tools.<tool>.egress` — operator-declared hosts merged into any run that *grants* that tool (task tier + oneshot facade). Generalizes the old web_search-only `task_default_allow` (dual-read). Retires debt Item 52.
- **User-configurable default `/task` grant** (`execution.task.default_grant`, Item 58). An AgentSpec-shaped `{tools?, network?, budget?}` that seeds a bare `/task "<desc>"` so a user can declare "these are the tools I normally want" and their environment just works — instead of every bare task 422-ing for a missing grant. A **new precedence layer**, not a new power: it slots *below* an explicit request/spec/skill/profile and *above* the built-in empty default, and the resolved grant — whatever its source — still passes every unchanged clamp (shell rejected, egress intersected with `execution.egress_ceiling`, tool kill-switches, provider validation, child ⊆ parent for spawns), so it can never escalate capability past the operator's envelope. `execution.task.allow_user_default` (operator bool, default true) disables it entirely for a locked-down deployment (fail-closed — a bare task keeps 422-ing). An explicit `--tools`/`--profile` still wins (replace/narrow). Coder baseline example: `default_grant.tools = ["web_search"]`.
- **`/task` and `/run` in the Rich and Textual TUIs (T8b — the port is done).** Both commands existed only in `web/shared/commands.js` and were deliberately absent from `CommandFactory`, because the TUIs have no channel to a `ppxai-server`. The transport question in `plan-task-command-sequencing.md` §T8b resolved in favour of **embed**: `build_task_runner` was extracted to `ppxai/engine/task_runner.py`, `ppxai/engine/task_backend.py` drives the same registry and the same sandbox in-process, and the TUIs became peers of the HTTP clients rather than a second implementation of them. **Gating is per VERB on a capability, not per client on a name** — `launch` and `resume` schedule an `asyncio.Task` and need a live event loop; `ls`/`get`/`watch`/`cancel`/`collect`/`respond` are synchronous registry operations. So **Textual has the full set** and **Rich has everything except `launch`/`resume`**, with a message naming the reason instead of the command being absent (reads as "unknown command") or present and silently minting runs that never advance. Asking `asyncio.get_running_loop()` asks the actual requirement rather than a proxy for it, so Rich starts working the moment its blocking-prompt main-loop question is settled, with no change to `/task`. Textual also gained a parked-run consent screen (one prompt per park token; Escape defers rather than denies) and a focus opt-out so `/task ls` does not steal the cursor.
- **Live web-app E2E suite** (`tests/e2e/live-app.spec.ts`). Every other spec in that directory drives a static `file://` harness; this one runs the REAL web UI against a REAL `ppxai-server`, covering the wiring harnesses can't see — command-envelope round-trips, SSE, and the AppState-driven badges. Opt-in via `npm run test:live` (starts the working-tree server, not the installed binary); `PPXAI_E2E_PROVIDER=<name>` enables the LLM-dependent steps, which otherwise skip.
- Run audit: `tool_call` events carry a truncated args snapshot, and a `run_usage` event records per-run tokens + tool cost + backend.

### Changed

- **Every `/v1/oneshot` is now a registry run.** The plain (non-enriched) path executes as a real `kind=oneshot` registry run, exactly like the enriched facade and `/v1/agent/run` — the direct non-registry code path is **deleted**, so the whole one-off tier has one execution path. **The wire contract is unchanged** (same request, same response envelope byte-for-byte, same 502 error contract on provider failure — gateway-smoke 6/6 against the live server). Around it: every oneshot call now leaves an auditable record in `~/.ppxai/runs/<id>/` and appears in `/run ls` (status `completed` — a plain oneshot never holds; the HTTP response *is* the collect), subject to the standard retention reaper; a client disconnect cancels the run cooperatively instead of abandoning the provider call; and native grounding rides along by construction, since the run's provider is built through the same construction site that applies `execution.run.grounding`.
- **`tools.web_search.preferred` is now an ORDERING, not a pin** (ADR 0009 step ④). ⚠️ **Behavior change for existing configs.** A concrete `preferred` (`"perplexity"` / `"gemini"` / `"duckduckgo"`) used to be a hard pin: no cross-backend fallback, egress narrowed to that backend. It now means *first-choice-then-fall-back* — the chain stays live and the egress set is the full backend superset, i.e. **egress widens on upgrade**. To keep the old pin, add **`strict: true`** in the *same scope* as `preferred`. The two resolve **together, as one scoped tuple**: the provider block (`providers.<name>.web_search`) owns both fields iff it states `preferred`, otherwise the global block does; a per-provider `strict` without a per-provider `preferred` is a dead key. `/doctor` reports the resolved tuple per scope and flags a concrete `preferred` without `strict`, a dead per-provider `strict`, and `strict` combined with enrichment (legal, but one backend outage returns the run to closed-book).
- One shared backend resolver now feeds **both** the call-time search chain and the AC-2 egress enumeration — they previously read config differently, so a per-provider override could select one backend while egress narrowed to another. Provider context is threaded through `NetworkPolicy` into `tool_targets`, so per-provider tuples resolve identically at both sites. Fallback ordering is honest now: a failed `preferred=gemini` tries perplexity before DuckDuckGo (previously it skipped straight to DDG).
- `POST /v1/agent/run` (in-development surface, not the frozen `/v1/oneshot`): runs are stamped `kind=oneshot`; a successful run now **holds** its result (`completed_pending_ack`) until collected, like `/task`; and the loopback auth exemption applies **only while `execution.run.web_search` is off** — once the config grants web_search, the endpoint is a capability and requires a bearer even from localhost.
- `get_weather` is now https-only; the plain-http wttr.in fallback that made it un-allowlistable is removed.
- `scripts/gateway-smoke.py` updated to the collect contract it had missed: under `execution.collect: "yes"` (default) a `/v1/agent/run` result is held, so the smoke now acks it to `finalized` (and accepts straight-`completed` under `auto`/`no`). It also passes `--port` through to the server it spawns.
...
- **Empty-content assistant turns are no longer persisted or sent (Item 44).** When the empty-response auto-retry budget exhausted, both chat loops persisted an empty-content `assistant` message (and left the synthetic retry nudge in history); on the next turn Perplexity Sonar rejected the resent empty turn with `{'message':'Message content was empty','type':'invalid_message'}` (400, verified live 2026-07-13). Root cause: nudge retries were treated as valid conversation turns rather than transient repair. Fixed structurally: `finalize_empty_response()` rolls back the transient nudge and coalesces empty content to a visible sentinel on every exhaustion path (unifying both loops with the already-correct tool-iteration path); a pure `strip_empty_assistant()` pass, wired into `validate_and_fix_alternation`, self-heals sessions saved by earlier builds that already carry an empty turn; and a composed `sanitize_outbound()` (orphan + empty passes) guards the wire copy at a single call site. Native tool-calling turns (empty text but `tool_calls` present) are preserved. (`engine/chat.py`, `engine/session.py`)
- **`/clear` left the status-bar `Ctx:` badge stale (Item 48) — fixed in all four clients.** `context_percentage` is refreshed by the engine's messages-changed fan-out, so `/clear`, `/compact`, session load and rollback all update it (engine + Rich). **ppxaide** gains the live `Ctx` badge with Rich-parity thresholds (`~` at ≥80%, `!` at ≥100%, hidden on an empty session). **Web + VSCode** now receive the value as a push: the terminal `stream_end` SSE event carries `context_percentage` in its metadata (additive — alongside the existing `usage`), and out-of-band changes (`/clear`, `/compact`, load) emit one discrete `state_sync` through the command envelope. The field is deliberately **not** in the `state_sync` whitelist — no per-message push traffic.
- **Clear buttons bypassed the command envelope.** The web Clear button, the VSCode Clear button, and the `ppxai.clearHistory` palette command called `POST /sessions/clear` directly, while a typed `/clear` went through `POST /command/clear`. The bespoke call discards the response body — which is where the envelope's `events[]` live — so server-pushed AppState had to be re-fetched by hand at each call site, and a missed one meant a stale badge (this is what kept Item 48's staleness alive in the buttons after the typed command was fixed). All three now dispatch `/clear` through the envelope, so pushed state updates itself; VSCode gained a single `clearConversation()` path shared by both entry points. The live E2E suite carries a regression fence asserting the button hits `POST /command/clear` and never `POST /sessions/clear`.
- **Config-error fail-safe was incomplete for `execution.run.grounding`.** When the config source itself could not be read, `get_execution_run_config()` still consulted the *legacy* `tools.web_search.oneshot_grounding` key — a second, still-readable source — so a box whose config failed to load could keep provider-native search ON while every other `execution.*` knob correctly fell back to off. Both keys now resolve to `false` when the config is unreadable (an absent `execution` block is still normal and resolves defaults as before). A capability must not survive the failure of the config that governs it.
- **Structured output silently disabled Gemini grounding** — a regression inside this cycle's own `response_format` fix. Suppressing Google Search grounding whenever *any* `response_format` was set generalized the function-declaration conflict (real, and documented at the top of `gemini.py`) to `response_schema`, which does not share it. Verified against the live API on `gemini-3.1-pro-preview` (2026-08-09): `google_search + response_mime_type` and `google_search + response_mime_type + response_schema` are **both accepted**; only function declarations conflict. The cost was worse than the bug it rode in on — a caller combining `response_format` with `execution.run.grounding` kept its JSON and quietly lost its search, where the original defect at least failed visibly. Affected web, VSCode and the `/v1/oneshot` consumers alike.
- **The browser terminal had no interactive shell.** No command history, no readline line editing, no tab completion — two independent bugs. `get_shell_config()` silently dropped `shell_bin` and `login_shell`, so an operator setting `tools.shell.shell_bin` (the coder deployment sets `/bin/bash` + login) was ignored; and the fallback landed on `/bin/sh`, which is dash on Debian/Ubuntu and has none of those features regardless of flags. Config now steers the terminal, the unset case prefers bash (config → `$SHELL` → bash → `/bin/sh`), and the PTY child launches interactive (`-i`) and login (`-l`) with a seeded writable `HISTFILE`, so history survives a fresh container HOME with no `.bashrc`.
- **Office preview 500'd when LibreOffice was present but could not render.** The Ubuntu LibreOffice **snap** runs under strict confinement and cannot read outside `$HOME`, so converting a file under `/tmp` fails *with exit code 0* — no output, no error signal. `_libreoffice_available()` said yes, the render produced nothing, and `/files/preview` raised a hard 500, breaking its documented "never 500 for a preview we can't rasterize" contract. Present-but-yielding-nothing now falls through to the same extracted-text degrade the LibreOffice-missing path uses, which helps any host with a confined or broken install, not just snap dev boxes. A new `libreoffice_can_read(dir)` capability probe replaces presence-checking in the tests.
...
- Tests: `tests/test_orphan_toolcalls_regression.py` (13) — prompt-preservation, genuine-draft-still-dropped, pure-helper, mid-loop-guard integration (orphan); pure strips + composition, self-heal on load, empty-strip-exposed trailing-user preservation, producer nudge-rollback + sentinel + narrow-guard (empty-content / Item 44).
- **Debt closed this cycle:** Item 48 (`Ctx:` badge), Item 52 (sealed-task egress gate), Item 53 (task execution profiles). The debt inventory was slimmed ~26%, with closed items archived to `docs/archive/DEBT-INVENTORY-CLOSED.md`. Items 54 and 55 (Gemini and OpenAI provider-fleet refresh) were filed.
- **Config migration direction:** ADR 0010 establishes `execution.*` as the third top-level config axis (alongside `providers.*` and `tools.*`). The keys introduced here — `execution.run.*`, `execution.profiles`, `execution.egress_ceiling`, `execution.collect` — are that axis landing incrementally. **Dual-read is the exception:** only `execution.run.grounding` falls back to a legacy key (`tools.web_search.oneshot_grounding`), and `tools.<tool>.egress` dual-reads the old `task_default_allow`. The `execution.task.*` move above is a **clean break with no dual-read** — see the migration table in *Config shape (BREAKING — ADR 0010)*.
- **Coder image:** rtk pinned 0.39.0 → 0.45.0. Verified as a drop-in — the musl tarball downloads, runs, and rewrites through the same `rtk hook check` interface the shell-wrapper framework uses, so the in-image binary and the engine-side rewrite hook stay lined up.
...
> ⚠️ **PREVIEW / EXPERIMENTAL — agents API in development, NOT sealed.** The tool-capable tier (`POST /v1/agent/task`) is **default-off** (`tools.agent.task_tier_enabled`) and sandboxed **in-process only** — OS-level isolation (ADR 0003 tier-d) and DNS-rebinding/TOCTOU egress defense are **deferred**. Treat `network.allow_outbound` allowlists as *trusted operator input*, **not** a boundary against hostile/untrusted input; do not expose `/v1/agent/task` to untrusted input in this release. The whole `/v1/agent/*` + `/v1/tokens` surface is **exempt from the v1 stability contract** until sealed (see `docs/api-gateway.md`). Full per-item caveats: `docs/release-notes-v1.19.0.md`.

The v1 API gateway shape (`POST /v1/oneshot`, bearer auth, response shape) is **preserved** for ppxai-sre and other consumers — all gateway changes are additive and default-off.

### Added

- **Agent platform Stage 2** (`engine/agent_runs.py` `AgentRunRegistry`, `server/routes/agent_v1.py`). Increments 1–9 + post-Inc-9 hardening §A–§N: run lifecycle (`POST /v1/agent/run`, `GET /v1/agent/runs[/<id>]`), background execution, `events.jsonl` + monitor SSE (`?live=1`, level/category filters), capability-grant tool allowlist (AC-1, `ScopedToolManager`), egress allowlist (AC-2, `engine/tools/network_policy.py`), budgets + cooperative cancel + conditional-resume checkpoint, `spawn_subagent` (N=1, depth=1, consent-gated), `/v1/tokens` + pluggable secret sources + per-run owner authz, and an AppState `background_agents` mirror. Two tiers locked at the URL level: `POST /v1/agent/run` (tool-free oneshot) vs `POST /v1/agent/task` (tool-capable, sandboxed).
- **`/task` command family (web + VSCode, build plan T1–T8a).** The tool-capable tier's client: `/task run "<desc>" --tools a,b,c [--spec name] [--skill name] [--allow host] [--provider p] [--model m] [--budget iters=,time=,tokens=] [--work-dir path] [--system "…"]` plus `ls`/`show`/`watch`/`cancel`/`respond`/`ack`/`resume`/`help`, entirely over the `/v1/agent/*` routes. Web: `TaskController`/`TaskRunView` extend the `/agentrun` controllers via duck-typed seams (grant/egress/budget chips, live events log, Cancel/Collect/Resume). VSCode (T8a): `taskController.ts` with verb/endpoint/status **parity sentinels** against the web client, a typed `/v1/agent/*` httpClient slice, and consent via native QuickPick. The TUI port (T8b) is **parked** pending a transport decision. See the new **[Task Agent User Guide](docs/task-agent-guide.md)**.
- **Run lifecycle (T5–T7).** Interactive consent parks (`waiting` state + resume token + `POST /runs/{id}/respond`, TTL fail-closed deny, consent card / QuickPick); two-phase termination (`completed_pending_ack` + `POST /runs/{id}/ack` → `finalized`, Collect button, lazy retention reaper); conditional interrupted-resume (`POST /runs/{id}/resume` gated on the `resume_refusal` decision matrix, restart-orphan sweep at registry construction, Resume button).
- **Spec files (T3):** `--spec <name>` under `tools.agent.sandbox.specs_dir` (`engine/agent_spec.py`; `.md` front-matter or `.json`), precedence request > spec > skill > `default_subagent`, ceiling-clamped (no shell, tier gate).
- **Skills (T4):** `--skill <name>` under `sandbox.skills_dir` (`engine/agent_skill.py`): `SKILL.md` grant + `references/` mounted into the run's read scope; skills compose.
- **Filesystem seal — `tools.agent.sandbox` (build plan T2).** Opt-in per-run read/write confinement: `FilesystemPolicy` (`engine/tools/filesystem_policy.py`) confines read-class tools to `read_paths.allow` (+ `skills_dir`/`specs_dir`/workdir) and write-class tools to a per-run workdir; deny-wins globs, boundary-anchored, symlink-escape safe, enforced at the `ScopedToolManager` chokepoint (off-scope → `path_denied` event). **Default `enforcement:"off"`** — non-breaking; the `container` sub-block schema is defined but inert (tier-d).
- **Per-run workdir intent:** `workdir` on `POST /v1/agent/task` — clients thread the session working dir (`--work-dir` overrides); unsealed default is `server.working_dir` → home (never the server launch dir); sealed runs keep their jail and flag `workdir_ignored`.
- **Client `/token` bearer management (Item 40):** `status|set|mint|clear` in web + VSCode — web `localStorage`, VSCode `SecretStorage` (shared with the "ppxai: Set API Token" palette entry); bearer scoped to `/v1/*`; 401s from agent verbs point at the fix.
- **Web `/agentrun` UX.** Background oneshot runs render into a right-panel `AgentRunView` (one pane per `run_id`); chat stays interactive; `/agentruns` lists runs. Logic in `web/shared/agent-run-controller.js`.
- **Opt-in oneshot grounding** (`tools.web_search.oneshot_grounding`, default off) — switches a *search-capable* provider into its own web search for `/v1/oneshot` + `/v1/agent/run` (Option A: no tool exposed, egress perimeter unchanged).
- **`gateway-smoke.py`** (`scripts/gateway-smoke.py`) — stdlib-only v1-surface acceptance for an installed binary: `/status`, `/v1/agent/runs`, `POST /v1/oneshot` shape, the `/v1/agent/run`→`completed` and `/v1/agent/task`→ack→`finalized` lifecycles; refuses to spawn over a held port. Wired into the build-install skill's step-8 acceptance.
...
- **Gemini:** google-genai unfrozen 1.56.0 → **2.11.0** (ceiling `<2.12.0`), KI-001 resolved via the benchmark gate; tool-schema sanitizer downgrades JSON Schema to Gemini's strict OpenAPI subset (`oneOf`→`anyOf`, list-type→nullable) so spawn-tool schemas no longer crash the request.
- **Gemini native tool loop (debt Item 41):** `_parse_function_call` threads a `tool_call_id` (synthesized when the SDK omits it) and `_convert_messages` maps the engine's native transcript onto Gemini's `function_call`/`function_response` parts (paired by function name) — activating `engine/chat.py`'s native pairing branch for Gemini instead of the synthetic "I'll use the X tool" text flattening. Dead `_filter_empty_parts` deleted. Benchmark gate 3× gemini-2.5-flash on 2.11.0: code editing 100/100/100, overall 80.7/72.6/73.8.
- **Desktop:** the web-UI installer is version-gated (`~/.ppxai/web/.installed-by` marker) — local web syncs survive relaunches instead of being clobbered on every desktop launch.
- Per-client completion gating: server-driven autocomplete no longer offers client-side commands (`/task`, `/token`, `/agentrun`) to clients that don't implement them.
...
- Filesystem-jail fail-closed on missing/aliased paths; deny-glob basename matching; two sandbox escapes from an external review round closed.
- Cancel cascades to in-flight sub-agents; parent-cancel propagates to an awaited child within ~100ms.
- Loopback UI auth exemption + `/v1/agent/run` carve-out so the token-less local browser can use the safe tier while the owner-scoped monitor channels + `/v1/tokens` stay bearer-gated even on loopback; `X-Forwarded-For` spoofing of the loopback check closed.
- Item 24: non-vision image attach fails loud (send blocked, actionable error) instead of feeding a hallucination placeholder; a shell-CLI consumption route surfaces the on-disk path for OCR/convert.
- `AgentRunView` holds its render state and rebuilds on mount, so RightPanelFrame re-mounts (open/back-forward/focus) restore the result instead of wiping it; the `/task` live-log DOM is bounded, not just its backing array.
- AC-2 backend-coverage test (`TestBackendCoverage`, debt Item 37c) now scans `web.py` too and catches `{scheme}://` f-string hosts (`wttr.in`) a bare `https?://` regex missed.
...
- CORS wildcard (`*`) removed + Host-header validation added (debt u); microk8s multi-tenant deploy gains an ingress NetworkPolicy for cross-tenant isolation.
...
- **Web markdown-preview DOM-XSS hardening.** The web file-tree markdown preview rendered `marked.parse()` output straight into `innerHTML`; marked v11 emits raw HTML verbatim (no built-in sanitize), so previewing a `.md` carrying `<img onerror=…>`, `<script>`, or a `javascript:` link could execute script in the app origin — and in the multi-tenant deployment that script can call the authenticated `/files/*` routes to exfiltrate `~/.ssh`/`.env`. Now the parsed HTML runs through **DOMPurify 3.4.10** (vendored at `ppxai/web/lib/dompurify.min.js`, Apache-2.0/MPL-2.0, loaded before app code) before `innerHTML`; if either lib is unavailable the view fails safe to escaped plain text. Verified via jsdom that the four payload classes (`<img onerror>`, `<script>`, `javascript:` link, `<iframe>`) are neutralized while benign markup (bold/code/links/images) survives. `markdown-file-view.js::_renderMarkdown`.
- **File-route home-dir confinement: prefix-confusion fix.** `_resolve_safe_path` and `/files/write` gated home-tree access with `str(path).startswith(str(home_dir))`, which let a sibling sharing a name prefix (e.g. `/home/userEVIL/.ssh/id_rsa` vs `/home/user`) pass. Replaced with `_within_tree()`, a component-wise subtree test (`path == base or base in path.parents`) that also rejects ancestors. New unit tests in `tests/test_files_route.py::TestWithinTreeConfinement` (5 cases). `ppxai/server/routes/files.py`.
- **Upload symlink-write defense-in-depth.** `POST /files/upload` computes `dest = <checked-dir>/<basename>` then `open(dest, "wb")`. If `dest` was a pre-planted symlink pointing outside the allowed tree the write would follow it. The handler now rejects a symlinked destination with 403 (the security-checked parent is already canonical, so the basename symlink was the only residual escape). New tests in `tests/test_files_upload.py::TestSymlinkedDestination`. `ppxai/server/routes/files.py`.
- **`AUTH_MODE=stub` loud startup warning.** Stub mode (the default) issues a valid signed cookie for any username with no password — the C1 gate then enforces cookie integrity but cannot enforce identity, so it is POC/dev only. The session-manager now logs an unmistakable startup `WARNING` when `AUTH_MODE=stub`, directing operators to set `AUTH_MODE=ldap` for any multi-tenant/production deployment. `deploy/images/session-manager/main.py`.
...
- **`/files/download?path=` server endpoint.** Streams raw bytes with `Content-Disposition: attachment; filename="<basename>"` so browsers fire their native download dialog. Reuses the same `_resolve_safe_path` security check as `/files/read` + `/files/preview` (working_dir tree OR home_dir tree). Companion to the new download buttons in the file-tree rows + BaseView toolbar. Commit `60d57037`.

- **`OfficeFileView`** (`ppxai/web/components/views/office-file-view.js`). New BaseView subclass with three rendering branches: spreadsheet (xlsx/xls/csv via SheetJS client-side, sheet tabs + DataTableViewer per sheet), presentation (pptx/ppt with built-in slide nav + keyboard support), word (docx/doc PDF iframe). LibreOffice-missing path renders extracted text inline with an install-LibreOffice note. Static render helpers (`renderSheetJsInto`, `renderSlideNavInto`, `renderDocxPdfInto`) are shared with the chat-attachment renderers in `app.js` — one rendering implementation, two entry points. Commit `330acb60`, dispatcher wiring + dedupe in `62f485eb`.

- **Download buttons in every file-backed view + file tree.** File-tree rows show a `⬇` icon on hover/focus that triggers a native browser download via `Content-Disposition: attachment`; every BaseView subclass toolbar (image/pdf/markdown/data/office, plus code-editor in view mode) shows a `⬇ Download` button. One handler (`PpxaiApp.onFileDownload`), two call sites — uses a hidden `<a download>` click rather than `window.location` so the URL's Content-Disposition fires the dialog without navigating away. Commit `330acb60`.

- **`BaseView._renderToolbar` + `_wireDownloadButton` helpers.** Pre-v1.18.7 every view subclass wrote `<div class="rpf-view-toolbar">…</div>` literally inline, so adding a shared button needed surgery in every view. The helper centralises the shape: subclasses pass type-specific info text + extras; base assembles toolbar with download button when `getPath()` is non-null. 5 existing views (image/pdf/markdown/data/code-editor) retrofitted; behavior preserved. Commit `330acb60`.

- **`/files/read` xlsx/xls/csv branch.** Office spreadsheets now return `{type: "office_spreadsheet", content (base64), mime_type, size}` — same shape as the image/pdf branch — so the web client decodes base64 and feeds it to SheetJS for client-side rendering. PPTX/DOCX 400 error message now points clients at `/files/preview?path=` for the LibreOffice-conversion path. Commit `60d57037`.

- **Tests** for the new endpoints (`tests/test_files_preview_download.py`, 19 cases): office-spreadsheet branch of `/files/read` (xlsx + csv), PPTX/DOCX 400 hint, `/files/preview?path=` input validation, download happy + error paths (200 with attachment disposition, MIME lookup, 404, 403, 400-on-directory, 400-on-empty), and PPTX preview branches that runtime-branch on whether LibreOffice is installed on the test host. Pre-existing Windows-only CRLF flake in `test_absolute_path_returns_content` opportunistically fixed (used in-memory string length where the route returns on-disk byte count). 40/40 file-route tests pass. Commit `60d57037`.
...
- **`displayFileFromEvent` dispatcher routes office files to `OfficeFileView`** (`ppxai/web/app.js`). New branch in the if/elseif chain placed before the `dataExts` branch so `.csv` now routes to `OfficeFileView` (sheet tabs, sortable/filterable DataTableViewer) instead of `DataFileView`'s plain-text view. `dataExts` narrowed accordingly — csv removed, tsv/tab + structured-data formats (json/yaml/toml/hcl/tf/tfvars) stay. This is the actual fix to the file-tree office-doc preview regression. Commit `62f485eb`.

- **`PpxaiApp._render{Spreadsheet,Presentation,Word}Attachment` delegate to `OfficeFileView` static helpers** (`ppxai/web/app.js`). The three chat-attachment renderers each previously had ~75 LoC of inline SheetJS-render / slide-nav / PDF-iframe logic; now ~25 LoC each, with rendering itself going through `OfficeFileView.renderSheetJsInto` / `renderSlideNavInto` / `renderDocxPdfInto`. Visual output byte-identical to pre-refactor; same SheetJS config, same slide-nav HTML structure, same iframe. Net: ~150 LoC of inline rendering removed from `app.js`, plus the file-tree preview path and the chat-attachment preview path now share one render implementation. Commit `62f485eb`.
...
3910 collected (3907 passed, 3 skipped on Unix with `--all-extras`; exit 0) as of the v1.18.7 pre-tag full run. Grown from the v1.18.6 baseline of 3695 by this branch's additions — `/files/read` HTTP route tests (`d06c5ee2`), `file_tree.ignore_dirs` (`tests/test_file_tree_ignore_config.py`, 11), `/files/preview` + `/files/download` (`tests/test_files_preview_download.py`, 19), `POST /files/upload` (`tests/test_files_upload.py`), the k8s session-manager C1/H2 + LDAP auth quick-pass suite (`tests/test_session_manager_auth.py`, 29), and the post-security-review hardening tests (`TestWithinTreeConfinement` ×5, `TestSymlinkedDestination` ×2 in `09eae96e`). Skip count is platform- and extras-dependent: on Windows the `TestKillPreviewBackend` cases skip (can't `patch()` `os.getpgid` / `os.killpg`), and without the `[data]` / multipart extras the office + upload suites skip — so a non-`--all-extras` run reports more skips (e.g. base venv: 3841 passed / 7 skipped).
...
#### Documentation
- Updated [Agent Mode Guide](docs/session-agent-guide.md) with configuration section
...
- **Agent Mode Guide** - Comprehensive documentation at [docs/session-agent-guide.md](docs/session-agent-guide.md)
