...
- **Step-5 follow-ups, same day:** `/q` is now a registered alias of `/quit` (works in Rich too, not just Textual); `/help <cmd>` renders a "Subcommands" section for commands that declare them; `/tools help` and `/tools help editing` now work in Rich, Textual and web (previously VSCode-only); VSCode's `/tools`, `/context`, `/ls` and `/tree` output is now server-rendered via `POST /command/<name>` (format differs slightly from the old client-side rendering; `/ls "dir with spaces"` takes only the first token, matching web's existing behaviour) — `/checkpoint` is the one VSCode intercept that stays client-side for now: `/checkpoint clear`'s irreversible delete needs a confirmation, and the owner decided (2026-09-21) to build one that works in all four clients, including both TUIs, before `/checkpoint` migrates off the legacy table; implementation in progress. `/context clear` no longer leaves a stale Ctx% badge in web/VSCode; three stale `/tools agent` hints (removed by ADR 0011) are fixed.
