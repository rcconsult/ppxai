...
> ⚠️ **PREVIEW / EXPERIMENTAL — agents API in development, NOT sealed.** The agent platform ships as a preview. The tool-capable tier (`POST /v1/agent/task`) is **default-off** (`tools.agent.task_tier_enabled`) and sandboxed **in-process only** — OS-level isolation (ADR 0003 tier-d) and DNS-rebinding/TOCTOU egress defense are **deferred**. It is safe only for **trusted operators** (the task/grant is operator-authored); treat `network.allow_outbound` allowlists as *trusted operator input*, **not** a safety boundary against hostile/untrusted input. The whole `/v1/agent/*` surface is **exempt from the v1 stability contract** until designed, tested, validated, and sealed (see `docs/api-gateway.md`). The web `/agentrun` and `/task` UX are experimental; the `/task` TUI/VSCode clients and the interactive `respond`/`ack`/`resume` endpoints are not built yet. See the per-item caveats in `docs/release-notes-v1.19.0.md`.
...
- **`/task` command family (web, build plan T1).** The tool-capable tier's client: `/task run "<desc>" --tools a,b,c [--allow host] [--provider p] [--model m] [--budget iters=,time=,tokens=] [--system "…"]` plus `ls`/`show`/`watch`/`cancel`/`help`, entirely over the existing `/v1/agent/*` routes. `TaskController` extends `AgentRunController` and `TaskRunView` extends `AgentRunView` (grant/egress/budget chips, live events log, Cancel button) via duck-typed seams, so `/agentrun` is unchanged. See `web/shared/task-controller.js`, `web/components/views/task-run-view.js`.
- **Filesystem seal — `tools.agent.sandbox` (build plan T2).** Opt-in per-run read/write confinement for the tool-capable tier: `FilesystemPolicy` (`engine/tools/filesystem_policy.py`) confines read-class tools (`read_file`/`list_directory`/`search_files`) to `read_paths.allow` (+ `skills_dir`/`specs_dir`/workdir) and write-class tools (`write_file`/`apply_patch`/…) to a per-run workdir; deny-wins globs, boundary-anchored, symlink-escape safe. Enforced at the `ScopedToolManager` chokepoint (off-scope → `path_denied` event, tool never runs). **Default `enforcement:"off"`** — non-breaking; the `container` sub-block schema is defined but inert (tier-d). See `docs/agent-task-command-design.html` §6.
