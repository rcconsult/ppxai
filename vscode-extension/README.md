...
- **macOS (Apple Silicon):** `ppxai-server-macos-arm64` + `ppxai-1.12.5.vsix`
- **macOS (Intel):** `ppxai-server-macos-intel` + `ppxai-1.12.5.vsix`
- **Linux:** `ppxai-server-linux-amd64` + `ppxai-1.12.5.vsix`
- **Windows:** `ppxai-server-windows.exe` + `ppxai-1.12.5.vsix`
...
```bash
code --install-extension ppxai-1.12.5.vsix
```
