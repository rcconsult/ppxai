...
- **macOS (Apple Silicon):** `ppxai-server-macos-arm64` + `ppxai-{version}.vsix`
- **macOS (Intel):** `ppxai-server-macos-intel` + `ppxai-{version}.vsix`
- **Linux:** `ppxai-server-linux-amd64` + `ppxai-{version}.vsix`
- **Windows:** `ppxai-server-windows.exe` + `ppxai-{version}.vsix`
