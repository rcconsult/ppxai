...
- **macOS (Apple Silicon):** `ppxai-server-macos-arm64` + `ppxai-1.18.1.vsix`
- **macOS (Intel):** `ppxai-server-macos-intel` + `ppxai-1.18.1.vsix`
- **Linux:** `ppxai-server-linux-amd64` + `ppxai-1.18.1.vsix`
- **Windows:** `ppxai-server-windows.exe` + `ppxai-1.18.1.vsix`
...
```bash
code --install-extension ppxai-1.18.1.vsix
```
