...
>
> All nine increments are committed and live-trial-verified, plus post-Inc-9 hardening §A–§K (see [agent-platform-call-graphs.md](agent-platform-call-graphs.md)). Next: interactive sub-agent UX design — the `/task` command family + interactive `/respond`·`/ack`·`/resume` endpoints. See the design docs: [agent-task-lifecycle.html](agent-task-lifecycle.html) (run-state machine) and [agent-task-command-design.html](agent-task-command-design.html) (`/task` command surface).
>
