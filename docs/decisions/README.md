...
| [0007](0007-completion-first-class-service.md) | One command registry: completion, help and the roster from a single declaration | 🟡 Proposed — step 1 shipped v1.18.8; **step 2 open**, no target release (v1.19.0/.1/.2 all passed it). **Revised 2026-09-20: goal restated (define once, derive everywhere); roster decision changed from AppState push to a `GET /commands` pull endpoint.** Re-measured same day: still 0 `commands` imports on every span, but the edge closes a **package cycle** and `engine.completion`'s leaf status is now fenced by `TestEngineCompletionStaysALeaf` |
