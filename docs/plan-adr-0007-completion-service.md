...
| `tests/test_consumer_import_surface.py` | **New**, 8 tests (~18s, subprocess import closure). Pins what step 4 bought for an out-of-repo consumer: importing ppxai-sre's pinned surface (8 runtime + 4 seam-test-only `ppxai.engine`/`ppxai.config` imports; `engine.completion` is deliberately NOT asserted absent — step 4 made it harmless) loads **zero** `ppxai.commands` modules (both closure tiers measured at 77 modules). Positive control proves the probe can see a `ppxai.commands` import when there is one |
