...
|---|---|---|---|
| **R8** ✅ | 🟡 polish R3 | ~30 min + 9 tests | **Fixed 2026-04-19.** `_count_csv_rows_cols` now sniffs delimiter on the first 8 KB head sample and streams the rest through `TextIOWrapper(BytesIO(data))` so csv.reader pulls one row at a time. 10 MB CSV: tracemalloc peak drops from ~20 MB (full-file decode) to under 2 MB. 9 tests in `test_count_csv_streaming.py` (8 correctness + 1 memory-bound regression guard). |
...
| **R10** ✅ | 🟢 micro-perf | ~45 min + 10 tests | **Fixed 2026-04-18.** Cached the `_has_multimodal_attachments()` result on `SessionManager._multimodal_cache`. `add_message` flips the cache to `True` eagerly when the new message is multimodal; `remove_last_message` invalidates only if the popped message carried multimodal content; `clear` sets to `False` directly; `load`, `reset_for_model_switch`, and `validate_and_fix_alternation` invalidate to `None` when they mutate messages wholesale. Extracted `_message_has_multimodal()` module helper so the predicate has one definition. 500-msg session × 20 saves: 0 rescans after warm-up (was 20). 10 tests in `test_multimodal_cache.py`. |
...
| **R16** ✅ | 🟡 ppxaide event dispatch | ~1 hr + 2 tests | **Fixed 2026-04-18.** Refactored `ppxai/tui/stream_handler.py` dispatcher into two explicit sets: `EVENT_MAP` (routes to UI bus signals — now includes `CONTEXT_INJECTED`) and `NOOP_EVENTS` (intentionally ignored — `STATE_SYNC` via AppState observers, `AGENT_*` Rich-only, `STATUS` via INFO). Unknown types now log an actionable WARNING pointing back to this file. Drift test in `test_stream_handler_dispatch.py` fails if an `EventType` is added without updating either set. |
| **R17** ✅ | 🟡 Gemini NoneType 2nd path | ~30 min + 4 tests | **Verified fixed 2026-04-18.** Audit of `gemini.py` confirmed all 12 access sites to `.candidates` / `.parts` are guarded. Original fix commit `6feb406b` ("guard against None .parts in non-streaming response path") is in the v1.17.4 tag — the TODO's log entry was from a pre-tag binary. Added 4 regression tests in `test_gemini_null_parts.py` pinning the null-parts, no-candidates, null-content, and sync-helper paths so the guard can't silently regress again. |
