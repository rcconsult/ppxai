...
# Shipped table
#
# Item 65, step 1: the 65 rows are now stated as `ModelFacts` literals rather
# than derived from `BUILTIN_PROFILES` at import. Two overlapping vocabularies
# for one set of facts was the thing ADR 0012 set out to remove, and the seed
# bridge was the last of it.
#
# **Every row states every field.** The previous comment here argued a code
# row may rely on dataclass defaults, because it is "complete by construction"
# — true while rows are GENERATED, and false the moment a human edits one.
# `ToolCallingProfile.mode` defaulted to "native" while `ModelFacts.tool_mode`
# defaults to "prompt_based" (Q0a inverted it deliberately: unmeasured implies
# assume not capable). 52 of these 65 rows relied on the profile default, so a
# row that quietly omitted `tool_mode` would flip meaning with nothing to see
# in the diff. Restating every field costs verbosity and buys the property
# that a row means what it says.
#
# These literals were GENERATED from the seed output, not retyped, and
# `tests/test_model_facts_are_the_source.py::
# test_every_profile_row_flattens_to_its_facts_row` compares each one against
# the flattened original field for field — so this step provably changed no
# behaviour. `wire_protocol` is stated here too; it previously came from
...
SHIPPED_MODEL_FACTS: dict[str, ModelFacts] = {
    ...
    "gemini-2.5-flash-lite*": ModelFacts(
        wire_protocol='generate_content',
        tool_mode='native',
    ...
    "*/qwen3-next-80b*instruct*": ModelFacts(
        wire_protocol='chat_completions',
        tool_mode='native',
        fallback_on_empty=False,
        fallback_on_failure=False,
    ...
    "*/qwen3-next-80b*thinking*": ModelFacts(
        wire_protocol='chat_completions',
        tool_mode='native',
        fallback_on_empty=False,
        fallback_on_failure=False,
    ...
    "Qwen/Qwen3.6-35B-A3B-FP8*": ModelFacts(
        wire_protocol='chat_completions',
        tool_mode='native',
        fallback_on_empty=False,
        fallback_on_failure=False,
    ...
    "sonar-reasoning-pro*": ModelFacts(
        wire_protocol='chat_completions',
        tool_mode='auto',
        fallback_on_empty=False,
        fallback_on_failure=False,
    ...
    "sonar-deep-research*": ModelFacts(
        wire_protocol='chat_completions',
        tool_mode='prompt_based',
        fallback_on_empty=False,
        fallback_on_failure=False,
    ...
    # Gemini 3.6 / 3.7 / 3.8 Flash — MEASURED 2026-09-13 against the live
    # generateContent API, 3 trials each: native function calling, TWO parallel
    # calls per turn (finishReason STOP every trial), vision accepted, and
    # 1,048,576 / 65,536 token limits from ListModels.
    #
    # `tier` is INHERITED from gemini-3.5-flash ('S'), not benchmarked. The
    # table's own invariant requires a tier in {S,A,B,C,D} (see
    # test_every_row_is_internally_valid), so "unmeasured" is not a legal
    # value here. These are same-family successors that measured identically
    # to 3.5-flash on every capability probed, so 'S' is the defensible
    # carry-over — but it is an ASSUMPTION, and a benchmark run should
    # confirm or correct it. Do not cite it as measured.
    # Without these rows all three fall to the conservative floor
    # (tool_mode='prompt_based'), which would make the NEWEST models the
    # least capable agentically — the reason this table has to track the fleet.
    "gemini-3.8-flash*": ModelFacts(
        wire_protocol='generate_content',
        tool_mode='native',
        fallback_on_empty=True,
        fallback_on_failure=False,
    ...
    "gemini-3.1-flash-lite*": ModelFacts(
        wire_protocol='generate_content',
        tool_mode='native',
        fallback_on_empty=True,
        fallback_on_failure=True,
    ...
    "gemini-3.1-pro*customtools*": ModelFacts(
        wire_protocol='generate_content',
        tool_mode='native',
        fallback_on_empty=False,
        fallback_on_failure=False,
