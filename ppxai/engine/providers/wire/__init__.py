...
a provider declares the set it can speak and routing does the lookup. W2
lands `responses`; `chat_completions` and `generate_content` follow in W4,
at which point `openai_compat` and `gemini` stop being special cases.
"""
...
__all__ = ["ProtocolHandler", "ResponsesHandler", "HANDLERS", "get_handler"]
