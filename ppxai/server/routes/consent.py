...
from ..state import Session, get_session, get_session_manager
...
async def interrupt_stream(s: Session = Depends(get_session)):
...
async def respond_to_consent(
    ...
    logger.debug(f"Consent: POST /consent session={s.id} file={file_path} response={request.response} -> {response}")

    # Resolve via SessionManager (v1.13.10)
    resolved = await session_manager.resolve_consent(s.id, file_path, response)
    if resolved:
    ...
    logger.debug(f"Consent: NO PENDING REQUEST for session={s.id} file={file_path}")
...
async def respond_to_shell_consent(
    ...
    # Resolve via SessionManager (v1.13.10)
    resolved = await session_manager.resolve_shell_consent(s.id, command, response)
    if resolved:
        return {
            "command": command,
            "response": response,
